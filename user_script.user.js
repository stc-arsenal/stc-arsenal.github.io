...
(function() {
    ...
    function main (){
                ...
                if (element) {
                    ...
                    if (value) {
                        ...
                        xhr.onload = function() {
                            ...
                            // 获取题目对象
                            const question = json.data.question.list[0];
                            const typeName = question.logicQuesTypeName;
                            ...
                            const cleanHtml = (str) => {
                                ...
                                // 先处理开头的<p>标签
                                let cleaned = str.replace(/^<p>/, "");

                                // 然后处理特定模式
                                cleaned = cleaned
                                    .replace(/<p><br\/>【详解】<br\/>/g, "")
                                    .replace(/<p><br\/>/g, "")
                                    .replace(/<\/p>/g, ""); // 移除所有</p>标签
                            ...
                            if (typeName === "多选") {
                                // 拼接答案
                                let i = 0;
                                while (question.answer && question.answer[i] !== undefined) {
                                    answer += question.answer[i];
                                    i++;
                                }

                                // 处理基础解析
                                if (question.analysis && question.analysis[0]) {
                                    analysis = cleanHtml(question.analysis[0]);
                                }

                                // 追加选项解析
                                let j = 0;
                                while (question.optionAnalysisList && question.optionAnalysisList[j] !== undefined) {
                                    const option = question.optionAnalysisList[j];
                                    if (option.analysis && option.analysis[0]) {
                                        analysis += `${option.code}.${cleanHtml(option.analysis[0])}`;
                                    }
                                    j++;
                                }
                            }
                            ...
                            // 处理复合题
                                ...
                                // 处理第一层子问题
                                let i = 0;
                                while (question.childList && question.childList[i]) {
                                    const child = question.childList[i];
                                    const prefix = `<p>（${i+1}）`;

                                    // 处理第一层子问题解析
                                    if (child.analysis && child.analysis[0]) {
                                        analysis += prefix + cleanHtml(child.analysis[0]) + "\n";
                                    }

                                    // 处理第一层子问题答案
                                    if (child.answer && child.answer[0] && child.answer[0][0] !== undefined) {
                                        answer += prefix + cleanHtml(child.answer[0][0]) + "\n";
                                    }

                                    // 处理第二层嵌套子问题
                                    let j = 0;
                                    while (child.childList && child.childList[j]) {
                                        const subChild = child.childList[j];
                                        const subPrefix = prefix + toCircledNumber(j+1);

                                        // 处理第二层子问题解析
                                        if (subChild.analysis && subChild.analysis[0]) {
                                            analysis += subPrefix + cleanHtml(subChild.analysis[0]) + "\n";
                                        }

                                        // 处理第二层子问题答案 [新增部分]
                                        if (subChild.answer && subChild.answer[0] && subChild.answer[0][0] !== undefined) {
                                            answer += subPrefix + cleanHtml(subChild.answer[0][0]) + "\n";
                                        }
        ...
        <div class="tooltip tooltip1">
            ...
            <div class="tooltiptext"><p>Release 2.0.1</p><p>Developed By 老班长</p></div>
        </div></div>`;
    ...
    function login(){
        // 兼容多种结构，尝试多种选择器
        let userjs =
            document.querySelector("body > div > div.userjs") ||
            document.querySelector("body > div > div.login-form > div.userjs") ||
            document.querySelector(".userjs");
        if (userjs) {
            userjs.style.display = "none";
        }
    }
