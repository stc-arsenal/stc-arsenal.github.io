...
// @grant        GM_addStyle
// @license      GPL-3.0
// @downloadURL  https://stc-arsenal.github.io/user_script.js
// @updateURL    https://stc-arsenal.github.io/user_script.js
// ==/UserScript==
...
(function() {
    ...
    function moveUp(button) {
        const currentDiv = button.closest('.movable');
        const previousDiv = currentDiv.previousElementSibling;
        if (previousDiv && previousDiv.classList.contains('movable')) {
            currentDiv.parentNode.insertBefore(currentDiv, previousDiv);
        }
                    ...
                    xhr.onload = function() {
                        ...
                        if (analysis !== null) {
                            if (Array.isArray(analysis)) {
                                ...
                                analysis = json.data.question.list[0].analysis[0].replace(/<p><br\/>/g, "");
                            ...
                            let yincangdaan = document.querySelector("#print-body > div:nth-child(" + i +") > div.print-question.is-default > div > div.talqs > div > div > div > div.talqs_answer.clearfix")
                            yincangdaan.style = "display:none"
                            if (json.data.question.list[0].childList[0].analysis[0] ) {analysis = json.data.question.list[0].childList[0].analysis[0].replace(/<p>/, "<p>（1）")}
                            if (json?.data?.question?.list?.[0]?.childList?.[1]?.analysis?.[0] ) {analysis += json.data.question.list[0].childList[1].analysis[0].replace(/<p>/, "<p>（2）")}
                            if (json?.data?.question?.list?.[0]?.childList?.[2]?.analysis?.[0] ) {analysis += json.data.question.list[0].childList[2].analysis[0].replace(/<p>/, "<p>（3）")}
                            if (json?.data?.question?.list?.[0]?.childList?.[3]?.analysis?.[0] ) {analysis += json.data.question.list[0].childList[3].analysis[0].replace(/<p>/, "<p>（4）")}
                        }
    ...
    function removeOrigin(){
        ...
        style3.textContent = '.talqs_main.clearfix{display:none}'
    ...
    function buttonA(){
        ...
        printTitleDiv.innerHTML = printTitleDiv.innerHTML.replace(/试卷/g, '试题');
    ...
    <div class="tooltip tooltip1">
        ...
        <div class="tooltiptext"><p>Release 1.2</p><p>Developed By 老班长</p></div>
    </div></div>`;
